...
async function main() {
    ...
    const admin = await prisma.user.upsert({
        ...
        create: {
            ...
            passwordHash: hash,
            globalRole: client_1.GlobalRole.ADMIN,
            emailVerified: true,
            avatar: avatar('admin'),
            language: 'tr',
        },
    ...
    const organizer1 = await prisma.user.upsert({
        ...
        create: {
            ...
            passwordHash: hash,
            globalRole: client_1.GlobalRole.ORGANIZER,
            emailVerified: true,
            avatar: avatar('ayse'),
            language: 'tr',
        },
    ...
    const jury1 = await prisma.user.upsert({
        ...
        create: {
            ...
            passwordHash: hash,
            globalRole: client_1.GlobalRole.JURY,
            emailVerified: true,
    ...
    ].map(async (u) => prisma.user.upsert({
        where: { email: u.email },
        update: {},
        create: {
            ...u,
    ...
    // ─── BADGES ───────────────────────────────────────────────────────────────
    const badgeDefs = [
        { type: client_1.BadgeType.EARLY_ADOPTER, name: 'Erken Benimseyici', description: 'Platformun ilk kullanıcılarından biri.', icon: '🚀', color: '#F59E0B' },
        { type: client_1.BadgeType.ORGANIZER, name: 'Organizatör', description: 'En az bir yarışma düzenledi.', icon: '🎪', color: '#8B5CF6' },
        { type: client_1.BadgeType.JURY_MEMBER, name: 'Jüri Üyesi', description: 'Bir yarışmada jüri olarak görev yaptı.', icon: '⚖️', color: '#3B82F6' },
        { type: client_1.BadgeType.FIRST_WIN, name: 'İlk Zafer', description: 'İlk birinciliğini kazandı.', icon: '🥇', color: '#EAB308' },
        { type: client_1.BadgeType.FIRST_SUBMISSION, name: 'İlk Adım', description: 'İlk başvurusunu tamamladı.', icon: '📤', color: '#10B981' },
        { type: client_1.BadgeType.SOCIAL_SHARER, name: 'Sosyal Paylaşımcı', description: 'Bir yarışmayı sosyal medyada paylaştı.', icon: '📢', color: '#EC4899' },
        { type: client_1.BadgeType.TOP_3, name: 'İlk 3', description: 'Bir yarışmada ilk 3\'e girdi.', icon: '🏆', color: '#F97316' },
        { type: client_1.BadgeType.VETERAN, name: 'Veteran', description: '5\'ten fazla yarışmaya katıldı.', icon: '🎖️', color: '#6366F1' },
        { type: client_1.BadgeType.CONTEST_CREATOR, name: 'İçerik Üreticisi', description: '3 veya daha fazla yarışma düzenledi.', icon: '✨', color: '#14B8A6' },
    ];
    const badges = {};
    ...
    // ─── USER BADGES ──────────────────────────────────────────────────────────
    const awardBadge = async (userId, type) => prisma.userBadge.upsert({
        where: { userId_badgeId: { userId, badgeId: badges[type].id } },
        update: {},
        create: { userId, badgeId: badges[type].id },
    });
    await awardBadge(organizer1.id, client_1.BadgeType.EARLY_ADOPTER);
    await awardBadge(organizer1.id, client_1.BadgeType.ORGANIZER);
    await awardBadge(organizer1.id, client_1.BadgeType.CONTEST_CREATOR);
    await awardBadge(organizer2.id, client_1.BadgeType.ORGANIZER);
    await awardBadge(jury1.id, client_1.BadgeType.JURY_MEMBER);
    await awardBadge(jury2.id, client_1.BadgeType.JURY_MEMBER);
    await awardBadge(jury3.id, client_1.BadgeType.JURY_MEMBER);
    await awardBadge(participants[0].id, client_1.BadgeType.FIRST_SUBMISSION);
    await awardBadge(participants[0].id, client_1.BadgeType.TOP_3);
    await awardBadge(participants[0].id, client_1.BadgeType.FIRST_WIN);
    await awardBadge(participants[1].id, client_1.BadgeType.FIRST_SUBMISSION);
    await awardBadge(participants[1].id, client_1.BadgeType.TOP_3);
    await awardBadge(participants[2].id, client_1.BadgeType.FIRST_SUBMISSION);
    await awardBadge(participants[3].id, client_1.BadgeType.FIRST_SUBMISSION);
    await awardBadge(participants[4].id, client_1.BadgeType.VETERAN);
    ...
    const contestJudging = await prisma.contest.upsert({
        ...
        create: {
            ...
            reviewStatus: 'APPROVED',
            approvalMode: client_1.ApprovalMode.MANUAL,
    ...
    await prisma.contest.upsert({
        ...
        create: {
            ...
            createdById: organizer1.id,
            category: 'animation',
            status: client_1.ContestStatus.PENDING_APPROVAL,
    ...
    // ─── CONTEST MEMBERS ──────────────────────────────────────────────────────
    const cm = (userId, contestId, role) => prisma.contestMember.upsert({
    ...
    // ─── APPLICATIONS ─────────────────────────────────────────────────────────
    const makeApp = (userId, contestId, status, message) => prisma.contestApplication.upsert({
    ...
    // ─── SOCIAL: FOLLOWS ──────────────────────────────────────────────────────
    const follow = (followerId, followingId) => prisma.follow.upsert({
    ...
    // ─── SOCIAL: LIKES ────────────────────────────────────────────────────────
    const like = (userId, submissionId) => prisma.like.upsert({
    ...
    // ─── NOTIFICATIONS ────────────────────────────────────────────────────────
    const notify = (userId, type, title, body, link) => prisma.notification.create({ data: { userId, type, title, body, link } });
    await notify(participants[0].id, 'CONTEST_RESULT', '🏆 Yarışma Sonuçlandı!', 'Dijital İllüstrasyon Yarışması\'nda 1. oldunuz! Tebrikler.', '/contests/dijital-illustrasyon-2024');
    await notify(participants[1].id, 'CONTEST_RESULT', '🥈 Yarışma Sonuçlandı!', 'Dijital İllüstrasyon Yarışması\'nda 2. oldunuz.', '/contests/dijital-illustrasyon-2024');
    await notify(participants[2].id, 'CONTEST_RESULT', '🥉 Yarışma Sonuçlandı!', 'Dijital İllüstrasyon Yarışması\'nda 3. oldunuz.', '/contests/dijital-illustrasyon-2024');
    await notify(participants[3].id, 'APPLICATION_APPROVED', '✅ Başvurunuz Onaylandı', 'Sokak Fotoğrafçılığı Yarışması\'na başvurunuz onaylandı.', '/contests/sokak-fotografciligi-2025');
    await notify(testUser.id, 'APPLICATION_PENDING', '⏳ Başvurunuz İnceleniyor', 'Sokak Fotoğrafçılığı Yarışması başvurunuz inceleme aşamasında.');
    await notify(organizer1.id, 'ADMIN_ACTION', '📋 Yeni Başvuru', 'Animasyon Yarışması onay için bekliyor.', '/admin/contests');
    await notify(jury2.id, 'JURY_ASSIGNMENT', '⚖️ Jüri Görevi', 'Sokak Fotoğrafçılığı Yarışması\'nda jüri olarak atandınız.', '/contests/sokak-fotografciligi-2025');
    console.log('✅ Notifications created');
    // ─── CONTEST FAVORITES ────────────────────────────────────────────────────
    const fav = (userId, contestId) => prisma.contestFavorite.upsert({
        where: { userId_contestId: { userId, contestId } },
        update: {},
