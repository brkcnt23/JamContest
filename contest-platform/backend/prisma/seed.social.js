...
async function main() {
    ...
    const u = (username) => users.find(x => x.username === username);
    const c = (idx) => contests[idx % contests.length];
    // ─── EK SUBMISSIONS ───────────────────────────────────────────────────────
    ...
    for (const pair of likePairs) {
        ...
        if (!user || !sub || sub.userId === user.id)
            continue;
        try {
...
//# sourceMappingURL=seed.social.js.map